...
```
调整之后，提供了通用的方式可以直接在`vcl_recv`阶段直接不使用缓存，在开发者调用接口的时候，如果确认该请求是不可缓存的，则设置HTTP请求头的`Cache-Control`或者增加url query的方式，经过此调用之后，对于不可缓存的请求的处理已经是一种通用的模式，`varnish`对接的是多少个应用也不再需要重复配置了。对于上面这样的配置，存在两个问题（如果有恶意调用或者调用出错）：
